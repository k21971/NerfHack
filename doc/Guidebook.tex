...
{\it sortdiscoveries\/}
option); also for ``{\tt \#vanquished}'' and ``{\tt \#exiled}'' commands
...
%.lp
\item[\tb{\#exiled}]
List any monster types which have been exiled.
...
doing that for either {\\tt \#exiled} or {\\tt \#vanquished} changes the order for both.
\\
%.lp ""
If the sorting order is ``count high to low'' or ``count low to high''
(which are applicable for {\tt \#vanquished}), that will be ignored
for {\tt \#exiled} and alphabetical will be used instead.
The menu omits those two choices when used for {\tt \#exile}.
\\
%.lp ""
Autocompletes.
...
from the game without being killed (perhaps by exile, or by a mollified
...
The {\tt \#exiled} command shares this sorting order.
\\
%.lp ""
...
%.lp
\item[\tb{\#wizborn}]
Show monster birth, death, exile, and extinct statistics.
Debug mode only.
%.lp
\item[\tb{\#wizbury}]
...
to eliminate one or more species of monsters by exile; playing without
this feature is considered a challenge.  When the game offers you an
opportunity to exile monsters, you may respond with the monster type
...
enabled by setting the correspondingly named option in {\tt NERFHACKOPTIONS}
...
file, or in the ``{\tt NERFHACKOPTIONS}'' environment variable.
...
%.lp ""
On UNIX, Linux and macOS it is \mbox{``.nerfhackrc''} in the user's home
directory.
The file may not exist, but it is a normal ASCII text file and
can be created with any text editor.\\

%.lp ""
On Windows, the name is \mbox{``.nerfhackrc''} location in the folder
...
template for ths configuration file named \mbox{``.nerfhackrc.template''} in
\mbox{{``\%USERPROFILE\%\textbackslash NetHack\textbackslash''}}.
...
defaults to the location of the {\it NerfHack.exe\/} or {\it NerfHackW.exe\/} file
...
%.hn 2
\subsection*{Using the {\tt NERFHACKOPTIONS} environment variable}

%.pg
The NERFHACKOPTIONS variable is a comma-separated list of initial
...
%.SD i
\begin{verbatim}
    setenv NERFHACKOPTIONS "color,\!leg,name:Blue Meanie,fruit:lime"
\end{verbatim}
%.ED
...
%.SD i
\begin{verbatim}
    NERFHACKOPTIONS="color,!leg,name:Blue Meanie,fruit:lime"
...
%.pg
The NERFHACKOPTIONS value is effectively the same as a single OPTIONS
...
NERFHACKOPTIONS can be set to the full name of a configuration file you
want to use.
...
{\tt g} --- list monster species that have been exiled;\\
...
The listings of vanquished monsters and of exiled types can be sorted,
...
{\tt exiled} to {\it not disclose\/} and {\it not prompt\/},
...
and also for the {\tt \#exiled} command.
Persistent.
\\
%.lp ""
The possible values are:
...
\\
%note: 'A' and 'C' can be set in RC file or NERFHACKOPTIONS but not by 'O'
% {\tt A} ---
...
non-comment line.
For a comma-separated list in NERFHACKOPTIONS or an OPTIONS line in a
...
older than {\tt qt-5.9},
{\it statuslines\/}
can only be set in the run-time configuration file or via NERFHACKOPTIONS,
...
individually within your own nerfhack.rc file.
