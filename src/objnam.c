...
not_fully_identified(struct obj *otmp)
{
    ...
    if (otmp->rknown || !erosion_matters(otmp))
        return FALSE;
...
paydoname(struct obj *obj)
{
    ...
    boolean save_wizweight = iflags.invweight;
    ...
    iflags.suppress_price--;
    iflags.invweight = save_wizweight;
...
static const char *const special_subjs[] = {
    ...
    "nemesis", "whisperfeet", "thunderfists", 0
...
static const struct alt_spellings {
    ...
    { "gdsm", GRAY_DRAGON_SCALES },
    { "sdsm", SILVER_DRAGON_SCALES },
    ...
    { "pfsc",  RIN_PROTECTION_FROM_SHAPE_CHAN},
    { (const char *) 0, 0 },
};
...
readobjnam_postparse1(struct _readobjnam_data *d)
{
    ...
    if (!strcmpi(d->bp, "scales") && d->mntmp >= FIRST_DRAGON
        && d->mntmp <= LAST_DRAGON) {
        d->typ = mndx_to_dragon_scales(d->mntmp);
...
dragon_scales_color(struct obj *obj)
{
    ...
    const char *pmname = pm->pmnames[NEUTRAL];
    const char* endp = strstri(pmname, " dragon");
    if (!endp) {
    ...
    int colorlen = endp - pmname;
    strncpy(buf, pmname, colorlen);
    buf[colorlen] = '\0';
    return buf;
}
