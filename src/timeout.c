...
nh_timeout(void)
{
    ...
    if (u.uluck != baseluck) {
        ...
        if (svm.moves >= u.luckturn + timeout) {
            if (u.uluck > baseluck)
                u.uluck--;
            else if (u.uluck < baseluck)
                u.uluck++;
            u.luckturn = svm.moves;
        }
    }
    if (HInvis & TIMEOUT)
        reappear_dialogue();
    ...
    for (upp = u.uprops; upp < u.uprops + SIZE(u.uprops); upp++) {
        if (upp->intrinsic & HAVEPARTIAL)
            continue; /* partial intrinsics do not time out */
            ...
            if (timed_out) {
                ...
                case MAGICAL_BREATHING:
                    You_feel(Hallucination ? "your gills disappear."
                                           : "your lungs start working again.");
                    if (!Breathless) {
                        if (region_danger())
                            You("cough%s",
