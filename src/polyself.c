...
polyself(int psflags)
{
    ...
    boolean forcecontrol = ((psflags & POLY_CONTROLLED) != 0),
            ...
            draconian = (!uskin && armor_to_dragon(&gy.youmonst) != NON_PM),
            /* enchanting dragon scales while confused; polycontrol will only
             * allow declining to become dragon, won't allow turning into
             * arbitrary monster */
 ...
 do_merge:
            ...
            mntmp = armor_to_dragon(&gy.youmonst);
            if (controllable_poly) {
                Sprintf(buf, "Become %s?", an(pmname(&mons[mntmp], Ugender)));
                if (y_n(buf) != 'y') {
                    return;
                }
            }
...
polymon(int mntmp)
{
    ...
    if (uskin && mntmp != armor_to_dragon(&gy.youmonst))
        skinback(FALSE);
    break_armor();
    drop_weapon(1);
    ...
    if (amorphous(gy.youmonst.data) || is_whirly(gy.youmonst.data)
        || unsolid(gy.youmonst.data) || nolimbs(gy.youmonst.data)) {
        if (Punished) {
            You("%s out of the iron chain.",
                (nolimbs(gy.youmonst.data)
                 && gy.youmonst.data->msize > MZ_MEDIUM) ? "break" : "slip");
            unpunish();
 ...
 */
int
armor_to_dragon(struct monst *mon)
{
