...
#undef add_ac
    ...
    case S_GIANT:
        if (ptr == &mons[PM_MINOTAUR] && ptr == &mons[PM_ELDER_MINOTAUR]) {
            if (!rn2(5) || (gi.in_mklev && Is_earthlevel(&u.uz)))
...
mk_gen_ok(int mndx, unsigned mvflagsmask, unsigned genomask)
{
    ...
    if (is_dino(ptr) && !Role_if(PM_CAVE_DWELLER))
        return FALSE;
    /* Prevent insane difficulty monsters in the mines */
    if (In_mines(&u.uz)
        && (ptr == &mons[PM_ALHOON] || ptr == &mons[PM_MASTER_MIND_FLAYER]
            || ptr == &mons[PM_DEEPER_ONE]|| ptr == &mons[PM_DEEPEST_ONE]))
        return FALSE;
    ...
    if (otmp) {
        ...
        if (is_prince(mtmp->data)) {
            ...
            if (otmp->oclass == POTION_CLASS 
                ...
                otmp->recharged = otmp->recharged > 6 ? 6 : otmp->recharged;
                otmp->spe -= rn2(3);
            }
        }
 ...
 assign_sym:
        if (s_sym == MAXOCLASSES) {
            static const int furnsyms[] = {
                ...
                S_altar, S_grave, S_throne, S_sink, S_toilet, S_forge
            };
