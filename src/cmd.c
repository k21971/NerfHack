...
find_remembered_stairs(boolean upstairs, coord *cc)
{
    ...
    y = iflags.travelcc.y;
    if (isok(x, y) &&
        (glyph_to_cmap(svl.level.locations[x][y].glyph) == stair ||
         glyph_to_cmap(svl.level.locations[x][y].glyph) == sladder ||
         glyph_to_cmap(svl.level.locations[x][y].glyph) == sbranch)) {
        cc->x = x;
        cc->y = y;
        return TRUE;
    }
