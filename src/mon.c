    ...
    int how)
{
    ...
    gd.disintegested = (how == AD_DGST || how == -AD_RBRE || how == AD_WTHR
...
unpoly_monster(struct monst *mtmp)
{
    ...
    if (is_were(mtmp->data) && !is_human(mtmp->data)
        ...
        mtmp->mhp = (mtmp->mhpmax / 2) + rn2(mtmp->mhpmax / 4 + 1);
        return;
    }
