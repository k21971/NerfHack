...
staticfn int getroomtype(coordxy, coordxy);
    ...
    glyph_info *glyphinfo)
{
    ...
    if (iflags.use_color && isok(x, y)) {
         ...
         */
        if (gmap->sym.symidx >= S_fountain + SYM_OFF_P
...
getroomtype(coordxy x, coordxy y)
{
    ...
    if (rno >= ROOMOFFSET) {
         ...
         * created. */
        return gi.in_mklev ? svr.rooms[rno - ROOMOFFSET].rtype
                           : svr.rooms[rno - ROOMOFFSET].orig_rtype;
    }
    /* not a room */
    return OROOM;
}
