...
weaphitmsg(struct obj *weap, struct monst *magr) {
    ...
    const boolean uhitm = (magr == &gy.youmonst);
...
barehitmsg(struct monst *magr) {
    const char *verb = 0;
    boolean uhitm = (magr == &gy.youmonst);
...
hitmsg(struct monst *mtmp, struct attack *mattk)
{
    ...
    if ((compat = could_seduce(mtmp, &gy.youmonst, mattk)) != 0
        ...
        switch (mattk->aatyp) {
        case AT_BITE:
            verb = has_beak(mtmp->data) ? "pecks" : "bites";
            break;
        case AT_KICK:
    ...
    struct attack *alt_attk_buf)
{
    ...
    else if (magr->data == &mons[PM_REVENANT]
        && attk->aatyp == AT_MAGC && attk->adtyp == AD_FIRE
        && udefend && m_next2u(magr)) {
        *alt_attk_buf = *attk;
        attk = alt_attk_buf;
        attk->aatyp = AT_WEAP;
        attk->adtyp = AD_PHYS;
...
mattacku(struct monst *mtmp)
{
    ...
    if (!range2 && marmf && marmf->otyp == STOMPING_BOOTS
        ...
        if (Upolyd && !Unchanging) {
            ...
            Strcpy(svk.killer.name, "getting stomped on");
            done(DIED);
        }
    }
