...
mon_wounds(struct monst *mon,
           boolean addspace, /* add ' ' to end of string */
           boolean clarity) /* end of game: don't decline for non-healer or
                             * obfuscate for hallucination */
{
    char *outbuf = nextmbuf();
    ...
    Sprintf(outbuf, "%s %s%s", adverb, adjective, addspace ? " " : "");
    return outbuf;
}
...
print_mon_wounded(struct monst *mon, int pre_wound_hp)
{
    ...
    m_old_wounds = mon_wounds(mon, FALSE, FALSE);
    mon->mhp = mcurrhp;
